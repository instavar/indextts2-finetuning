#!/usr/bin/env python3
"""Validate one content-bound HTTP observation against its matched CLI row."""
...
def main() -> int:
    ...
    if plan.get("instruction"):
        raise ValueError("HTTP and CLI parity is undefined for instruction-bearing rows")
    ...
    if receipt.get("runtime_id") != "indextts2_openai_compatible_http":
    ...
    report = {
        ...
        "boundary": (
            ...
            "It does not establish quality, throughput, concurrency safety, or production readiness."
        ),
    }
